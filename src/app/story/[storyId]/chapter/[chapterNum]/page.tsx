...
export default async function ChapterPage({ 
  ...
  params: Promise<{ storyId: string, chapterNum: string }> 
}) {
  
  // 1. "Deselvolvemos" los parámetros para obtener los IDs reales
  const { storyId, chapterNum } = await params;
  
  // 2. Llamamos a nuestra DB real
  ...
  const currentChapterIdx = parseInt(chapterNum) - 1;
  ...
  if (!chapter) {
    return (
      <div className="p-20 text-center font-sans">
        ...
        <Link href={`/story/${storyId}`} className="text-blue-500 underline mt-4 block">
      ...
      <div className="flex flex-col md:flex-row min-h-screen bg-white dark:bg-zinc-950 transition-colors">
        ...
        <main className="flex-1 max-w-3xl mx-auto p-6 md:p-12 font-serif flex flex-col dark:text-gray-200">
          ...
          <nav className="mb-12 flex justify-between items-center text-sm text-gray-500 uppercase tracking-widest font-sans">
            <Link href={`/story/${storyId}`} className="hover:text-black dark:hover:text-white font-bold transition-colors">
              ← Índice
            </Link>
          ...
          <article className="flex-grow">
            ...
            {/* Contenido con espaciado y capitular (Drop Cap) */}
            <div className="text-xl leading-relaxed space-y-8 first-letter:text-5xl first-letter:font-bold first-letter:mr-3 first-letter:float-left first-letter:text-black dark:first-letter:text-white">
          ...
          <footer className="mt-20 pt-8 border-t dark:border-zinc-800 flex justify-between font-sans items-center">
            {currentChapterIdx > 0 ? (
              <Link 
                href={`/story/${storyId}/chapter/${currentChapterIdx}`}
            ...
            {currentChapterIdx < totalChapters - 1 ? (
              <Link 
                href={`/story/${storyId}/chapter/${currentChapterIdx + 2}`}
