...
export default async function StoryDetailPage({ params }: { params: Promise<{ storyId: string }> }) {
  ...
  // ... resto del código igual usando 'storyId'

  return (
    <main className="max-w-4xl mx-auto p-6 md:p-16 font-serif">
      {/* Botón Volver */}
      <Link href="/" className="text-sm font-sans text-gray-500 hover:text-black dark:hover:text-white mb-8 block">
      ...
      <section className="border-b dark:border-zinc-800 pb-12">
        ...
        <div className="mt-8 flex gap-4 font-sans">
          <Link 
            ...
            className="bg-black dark:bg-white dark:text-black text-white px-8 py-3 rounded-md font-bold hover:opacity-90 transition-all"
          >
            Comenzar a leer
          </Link>
          <Link 
            href={`/story/${story.id}/add-chapter`}
            className="border border-gray-300 dark:border-zinc-700 px-8 py-3 rounded-md font-bold hover:bg-gray-50 dark:hover:bg-zinc-900 transition-all"
          >
            + Añadir Capítulo
          </Link>
        </div>
            ...
            {story.chapters.map((chapter, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <span className="text-2xl font-bold text-gray-200 dark:text-zinc-800 group-hover:text-black dark:group-hover:text-white">
